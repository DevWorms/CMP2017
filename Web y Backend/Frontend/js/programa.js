...
var init_url = encodeURIComponent(API_URL + 'programa/paginate/' + user_id + '/' + api_key);
...
$('document').ready(function() {
    ...
    $('#btn_crearEvento').click(function(){
        crear();
    });
});
    ...
    $.ajax({
        ...
        success :  function(response) {
            if (response.status == 1) {
                ...
                response.programas.forEach(function (expositor) {
    ...
    $.ajax({
        ...
        success :  function(response) {
            if (response.status == 1) {
                ...
                if (Object.keys(response.programas).length > 0) {
                    response.programas.forEach(function (expositor) {
                        var fecha = expositor.fecha;
                        if (expositor.hora_inicio) {
                            fecha = fecha + " " + expositor.hora_inicio
